/* C言語でニューラルネットワークを作る Lv.9 Activation層を導入する */
...
/* 推論 */
double predict(const Linear* pLinear, Activation* pAct, const Vector* x)
{
    ...
    /* 活性化関数を適用 */
    activation_forward(pAct, &y, &y);

    double result = y.data[0];
    ...
    const Matrix* x, const Vector* t, double lr, int epoch)
{
    for(int e = 0; e < epoch; e++)
    {
        for(int i = 0; i < x->rows; i++)
        {
            ...
            /* 推論 */
            double output = predict(pLinear, pAct, &input);
...
int main(void)
{
    ...
    for(int i = 0; i < x.rows; i++)
    {
        ...
        double output = predict(&linear, &act, &input);
